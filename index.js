...
function getAPIData()
{
    const APIKey = '893d73fd079692854dfdf19dde4329d1';
    ...
    then(response => response.json()).then(json => {
        if (json.cod == "404"){
            ...
            const sunriseDate = new Date((sunriseTime + json.timezone) * 1000).toUTCString(); 
            const sunsetDate = new Date((sunsetTime + json.timezone) * 1000).toUTCString(); 

            var sunsetHour = (new Date(sunsetDate).getUTCHours() > 12) ? (new Date(sunsetDate).getUTCHours() - 12) : new Date(sunsetDate).getUTCHours();
            ...
            switch(json.weather[0].main){
                ...
                case 'Clouds':
                    image.src = json.weather[0].description.startsWith('f') ?  'icons/Few-Clouds.png' : 'icons/Scattered-Clouds.png';
                    break;

                case 'Thunderstorm':
                    image.src = (json.weather[0].description.includes('rain') || json.weather[0].description.includes('drizzle')) ? 'icons/Thunderstorm-Rain.png' : 'icons/Thunderstorm.png';
                    break;

                case 'Drizzle':
                    image.src = json.weather[0].description.startsWith('l') ? 'icons/Light-Drizzle.png' : 'icons/Heavy-Drizzle.png';
                    break;
                ...
                case 'Snow':
                    ...
                    else if (json.son.weather[0].description.includes('rain') ||
                    (json.son.weather[0].description.includes('shower') && !json.son.weather[0].description.includes('sleet')))
            ...
            sunrise.innerHTML = `${zeroCorrector(new Date(sunriseDate).getUTCHours())}:${zeroCorrector(new Date(sunriseDate).getUTCMinutes())}AM`;
            sunset.innerHTML = `${zeroCorrector(sunsetHour)}:${zeroCorrector(new Date(sunsetDate).getUTCMinutes())}PM`;
